...
def kline_pro2(kline: List[dict],
              fx: List[dict] = None,
              bi: List[dict] = None,
              zs: List[dict] = None,
              xd: List[dict] = None,
    ...
    if bi:
        ...
        if zs:
            zs_data = []
            for i in zs:
                zs_data.append(opts.MarkAreaItem(x=(i['sdt'], i['edt']),y=(i['zd'],i['zg'])))
            # chart_zs = Line()
            # chart_zs.add_xaxis(xaxis_data=dts)
            # chart_zs.add_yaxis(series_name="", y_axis=k_data, itemstyle_opts=opts.LineStyleOpts(width=2))
